...
import {
  requestEmail,
  requestEmailSelector,
} from "../../features/Auth/RequestEmail";
...
export default function FoegetPassEmail() {
  ...
  <div className="flex flex-wrap">
    <div className="w-full lg:w-1/2 pad text-center">
      {isFetching ? (
        <div className="container w-full h-full flex justify-center items-center sm:px-4">
          <Loader />
        </div>
      ) : (
        <div className="container pt-5 rounded bg-slate-300 w-[90%] lg:w-[80%] min-h-60 mx-auto sm:px-4">
          <div>
            ...
            <form>
              ...
              />
              {errors.email && (
                <div className="font-semibold mt-2 text-sm text-red-500">
                  {errors.email}
                </div>
              )}
              <button
                ...
                className="px-9 rounded mt-5 py-4 dark:bg-DarkGray bg-primary text-white font-semibold"
              >
                Submit
              </button>
            </form>
