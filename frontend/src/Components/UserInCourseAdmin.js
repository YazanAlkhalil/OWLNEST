...
function UserInCourseAdmin({ user, index, fetchUsers, isOwner }) {
    ...
    async function deleteUser() {
        await fetchData({ url: "http://127.0.0.1:8000/api/delete_user/" + user.id + "/", method: "POST" })
        fetchUsers() 
    }
    async function changeRole() {
        if (user.roles.includes("admin")) {
                    ...
                    >
                        ...
                        <div className="flex border-l border-gray-200">
                            <button
                                onClick={
                                    async () => {
                                        await fetchData({ url: "http://127.0.0.1:8000/api/delete_admin/" + user.id + "/", method: "POST" })
                                        await fetchUsers()
                                        toast.dismiss(t.id)
                                    }}
            ...
            await fetchData({ url: "http://127.0.0.1:8000/api/add_admin/" + user.id + "/", method: "POST" })
        await fetchUsers()
    }
    return (
        <>
